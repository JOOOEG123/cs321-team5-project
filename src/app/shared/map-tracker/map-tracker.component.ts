...
export class MapTrackerComponent implements OnInit {
  ...
  private addAPin(vm : MapTrackerComponent, nextPin : Pin | undefined) {
  ...
  private addDialogProcessor() {
    const componentFactory = this.componentFactoryResolver.resolveComponentFactory(MapTrackerComponent);
  ...
  onClick(event:any) {
    ...
    if (this.pins.has(id) && !this.hasSelected) {
      ...
      let pin: Pin | undefined = this.pinInformation.pins.find(item => item.id === id) || undefined
      this.modalService.open('text-modal', pin ? pin.id : '', pin?.text);
